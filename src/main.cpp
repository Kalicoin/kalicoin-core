...
#if defined(NDEBUG)
# error "Kalicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x952f83ba8ffcfc31f370ae9ffe959cffe2761c0a7522a8a24fbcac3d4dd2af22");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kalicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kalicoin Signed Message:\n";
...
{
    // Kalicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Kalicoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Kalicoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kalicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kalicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kalicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7df5832f9552a32d0451c5c55eee4891e8dfb8f4897b695e6babd3eac7df2435");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sena Demands President's Rule, Says Goa 'Headless' Without Manohar Parrikar";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1520935281;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 538317;

        if (fTestNet)
        {
            block.nTime    = 1520935343;
            block.nNonce   = 239813453;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5bb15962d9213f153541b886abb571cd391c67fa43a45988ef3fafe39a53f05d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kalicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KalicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KalicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KalicoinMiner(CWallet *pwallet)
{
    printf("KalicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dcecoin-miner");
    ...
    try { loop {
        ...
        printf("Running KalicoinMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KalicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KalicoinMiner, pwallet));
}
